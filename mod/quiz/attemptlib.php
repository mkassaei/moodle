...
class quiz_attempt {
    ...
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_quiz_renderer $renderer, $seq) {
        ...
        if ($this->can_question_be_redone_now($slot)) {
            $redobutton = $renderer->redo_question_button($slot, $displayoptions->readonly);
            $output = preg_replace('~(<div class="state[^<]*</div>)~', '$1' . $redobutton, $output, 1, $count);
            if ($count = 0) {
                // If we failed to put the button in to the question, just tack it on the end.
                debugging('Failed to insert the redo button into the question\'s HTML.', DEBUG_DEVELOPER);
                $output .= $redobutton;
            }
        }
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $redolinks = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'quiz', $renderer->render($links)));
                $output = preg_replace('~(<h4 class="responsehistoryheader[^<]*</h4>)~', '$1' . $redolinks, $output, 1, $count);
                if ($count = 0) {
                    // If we failed to put the button in to the question, just tack it on the end.
                    debugging('Failed to insert the other redoes links into the question\'s HTML.', DEBUG_DEVELOPER);
                    $output .= $redolinks;
                }
            }
        }
